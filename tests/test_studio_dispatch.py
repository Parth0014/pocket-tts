...
from narration_studio.core import PreparedArtifact
...
class FakeDynamo:
    ...
    def update_item(self, **kwargs):
        ...
        if self.conflict:
            raise ConditionalFailure("synthetic")
        return {}
...
class FakeS3:
    ...
    def put_object(self, **kwargs):
        ...
        if key in self.objects:
            raise PreconditionFailure("synthetic")
...
def test_dispatch_pin_is_conditional_and_idempotent_shape():
    ...
    assert pinned.body == canonical_job_json(job)
    assert pinned.fingerprint == job_fingerprint(job)
    ...
    assert "attribute_exists(#generation_id)" in call["ConditionExpression"]
    assert "worker_job_body" in call["ExpressionAttributeNames"].values()
    assert (
        call["ExpressionAttributeValues"][":source_post_id"]["S"]
        == POST_ID
    )
    assert (
        call["ExpressionAttributeValues"][":source_content_hash"]["S"]
        == CONTENT_HASH
    )
...
def test_conflicting_generation_pin_fails_closed():
    store = DynamoGenerationDispatchStore(
        client=FakeDynamo(conflict=True),
        table_name="pocket-tts-app",
    )
    with pytest.raises(StudioDispatchConflictError):
        store.pin(
            room_id=ROOM_ID,
            job=make_job(),
            pinned_at=NOW,
...
def test_pinned_job_round_trip_is_verified():
    ...
    client = FakeDynamo(
        item={
            ...
            "job_id": {"S": JOB_ID},
            "worker_job_body": {"S": body},
            "worker_job_fingerprint": {"S": fingerprint},
        }
    )
    store = DynamoGenerationDispatchStore(
        client=client,
...
def test_fifo_publisher_uses_frozen_transport_values():
    ...
    publisher = SqsStudioJobPublisher(
        client=client,
        queue_url="https://example.invalid/studio.fifo",
    )
    assert publisher.publish(pinned) == "message-123"
    assert client.calls == [
        {
            "QueueUrl": "https://example.invalid/studio.fifo",
...
def test_worker_voice_store_is_immutable_and_retry_safe():
    ...
    store.put_immutable(artifact)
    store.put_immutable(artifact)

    with pytest.raises(WorkerVoiceArtifactConflictError):
        store.put_immutable(
            PreparedArtifact(
                key=artifact.key,
